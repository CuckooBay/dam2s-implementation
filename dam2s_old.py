...
class cmp(cooper.ConstrainedMinimizationProblem):
    ...
    def closure(self, v_features, d_features, labels, t_features, *trainable_params) -> CMPState:
        ...
        if torch.any(torch.isnan(v_error_slack)):
            raise TypeError("v_error_slack is nan")
        if torch.any(torch.isnan(d_error_slack)):
            raise TypeError("d_error_slack is nan")
        if torch.any(torch.isnan(v_proj_mat)):
            raise TypeError("v_proj_mat is nan")
        if torch.any(torch.isnan(d_proj_mat)):
            raise TypeError("d_proj_mat is nan")
        if torch.any(torch.isnan(v_svm_mat)):
            raise TypeError("v_svm_mat is nan")
        if torch.any(torch.isnan(v_svm_bias)):
            raise TypeError("v_svm_bias is nan")
        if torch.any(torch.isnan(d_svm_mat)):
            raise TypeError("d_svm_mat is nan")
        if torch.any(torch.isnan(d_svm_bias)):
            raise TypeError("d_svm_bias is nan")
